...
@Component({
    selector: 'jhi-monthly-task',
    templateUrl: './monthly-task.component.html'
})
export class MonthlyTaskComponent implements OnInit, OnDestroy {
    currentAccount: any;
    monthlyTasks: IMonthlyTask[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/monthly-task'], {
            queryParams: {
                page: this.page,
                size: this.itemsPerPage,
    ...
    ngOnInit() {
        ...
        this.registerChangeInMonthlyTasks();
    }
    ...
    trackId(index: number, item: IMonthlyTask) {
        return item.id;
    }

    registerChangeInMonthlyTasks() {
        this.eventSubscriber = this.eventManager.subscribe('monthlyTaskListModification', response => this.loadAll());
    }
    ...
    private paginateMonthlyTasks(data: IMonthlyTask[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        this.queryCount = this.totalItems;
        this.monthlyTasks = data;
    }
